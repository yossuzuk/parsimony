...
main(int argc, char *argv[])
{
	...
	struct codons codon[5][5][5];
        ...
        struct pair {int node1; int node2;};
...
printf("%d nucleotide sites\n", sitenum); fflush(stdout);
        ...
        while(fgets(buffer, MAXBUF, fin)!=NULL)
        {
                ...
                {
                        bufnodea1[aab]=buffer[aaa];
                        ++aab;
                }
                bufnodea1[aab]='\0';
                pairs[npairs].node1=atoi(bufnodea1);
                ...
                for(aaa; !isspace(buffer[aaa]); aaa++)
                {
                        bufnodea2[aab]=buffer[aaa];
                        ++aab;
                }
                bufnodea2[aab]='\0';
                pairs[npairs].node2=atoi(bufnodea2);
...
*/
			...
			for(aac=1; aac<pairs[aab].node1; aac++) /* pass sequence */
			{
			...
			for(aac=1; aac<pairs[aab].node2; aac++) /* pass sequence */
			{
				fgets(buffer, MAXBUF, fin);
			}
			fgets(buffer, MAXBUF, fin);
...
*/
                aaa/3+1, (wsynsite*3.0)/(wsynsite+wconsite), tsynchge, (wconsite*3.0)/(wsynsite+wconsite), tconchge); fflush(stdout);
        }
}
