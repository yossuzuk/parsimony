...
printf("%d nucleotide sites\n", sitenum); fflush(stdout);
        ...
        while(fgets(buffer, MAXBUF, fin)!=NULL)
        {
                ...
                aab=0;
                for(aaa; (!isspace(buffer[aaa]) && buffer[aaa]!='\n' && buffer[aaa]!='\0') ; aaa++)
                {
...
/****** koko ******/
for(yaa=0; yaa<npairs; yaa++)
{
printf("%d %d\n", atoi(pairs[yaa].name1), atoi(pairs[yaa].name2)); fflush(stdout);
}
/****** koko ******/
...
/****** koko ******
			if((atoi(pairs[aab].name1))<=15041 || ((atoi(pairs[aab].name2))<=15041))
			{
				goto owari1;
			}
****** koko ******/
...
****** koko ******/
/****** koko ******/
				if((atoi(pairs[aab].name1))==seqnum)
/****** koko ******/
				{
					atta1='y';
                        		...
                        		for(aac; buffer[aac]!='\n'; aac++) /****** Input the sequence ******/
                        		{
                                		if(!isspace(buffer[aac])) /****** Omitting spaces, gaps can be included ******/
                                		{
                                        		...
                                        		{ printf("unknown nucleotide %c\n", buffer[aac]); fflush(stdout); exit(1); }
                                		}
                        		}
                        		seq1[aad]='\0';
				}
...
****** koko ******/
/****** koko ******/
				if((atoi(pairs[aab].name2))==seqnum)
/****** koko ******/
				{
