...
int main(int argc, char *argv[])
{
	...
	int aasite[1000];
	int totalaa;
	char aa1, aa2, aa3, aa4;
	char sequonatta;
	int baa;
	struct sequons{char seq[101][5]; int num[101];};
...
/****** ******/
				...
				{
					cod[3]='\0';
					if(bufseq[696+aab]=='T' || bufseq[696+aab]=='t' || bufseq[696+aab]=='U' || bufseq[696+aab]=='u') { cod[aab]=1; }
					else if(bufseq[696+aab]=='C' || bufseq[696+aab]=='c') { cod[aab]=2; }
					else if(bufseq[696+aab]=='A' || bufseq[696+aab]=='a') { cod[aab]=3; }
					else if(bufseq[696+aab]=='G' || bufseq[696+aab]=='g') { cod[aab]=4; }
					else { aminoacid='X'; goto frame1owari; }
				}
				aa4=codon[cod[0]][cod[1]][cod[2]].aa;
	...
	for(aaa=1; aaa<=output; aaa++)
	{
		if(1<=aaa && aaa<=17)
		{
			...
			if(aasite[aaa]>0)
			{
				...
				{
					printf(" %c%c%c%c %d", sequon[aaa].seq[baa][0], sequon[aaa].seq[baa][1], sequon[aaa].seq[baa][2], sequon[aaa].seq[baa][3], sequon[aaa].num[baa]); fflush(stdout);
				}
				printf("\n"); fflush(stdout);
			}
		}
