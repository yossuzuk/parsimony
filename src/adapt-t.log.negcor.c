...
******/
	...
	printf("Cod.#\tone-tail-posp\tone-tail-negp\ttwo-tail-p\t\tSelect\ttwo-tail-1-p\n"); fflush(stdout);

	while(fgets(buffer, MAXBUF, fin1)!=NULL)
	{
		if(buffer[0]=='C') { ; }
		else if(isdigit(buffer[0])) /* Site number */
		{
			aaa=0;
			aab=0;
			...
			if(isdigit(buffer[aaa])) /* If results exists */
			{
				for(aaa; !isspace(buffer[aaa]); aaa++) { ; } /* Skipping the assignment number */
				for(aaa; isspace(buffer[aaa]); aaa++) { ; } /* Skipping the spaces */
...
/******/
				...
				aab=0;
				for(aaa; !isspace(buffer[aaa]); aaa++) /* The number of synonymous site */
				{
				...
				for(aaa; isspace(buffer[aaa]); aaa++) { ; } /* Skipping the spaces */
				aab=0;
				for(aaa; !isspace(buffer[aaa]); aaa++) /* The number of synonymous change */
				{
				...
				for(aaa; isspace(buffer[aaa]); aaa++) { ; } /* Skipping the spaces */
				aab=0;
				for(aaa; !isspace(buffer[aaa]); aaa++) /* Getting the number of synonymous substitutions */
				{
				...
				for(aaa; isspace(buffer[aaa]); aaa++) { ; } /* Skipping the spaces */
				aab=0;
				for(aaa; !isspace(buffer[aaa]); aaa++) /* Skipping the number of nonsynonymous site */
				{
				...
				for(aaa; isspace(buffer[aaa]); aaa++) { ; } /* Skipping the spaces */
				aab=0;
				for(aaa; !isspace(buffer[aaa]); aaa++) /* Skipping the number of nonsynonymous change */
				{
				...
				for(aaa; isspace(buffer[aaa]); aaa++) { ; } /* Skipping the spaces */
				aab=0;
				for(aaa; !isspace(buffer[aaa]); aaa++) /* Getting the number of nonsynonymous substitutions */
				{
...
/******
					...
					if(tvalue>1.96)
					{ printf("\t\t%f \tP\t", tvalue); fflush(stdout); }
					else if((-1.96<=tvalue) && (tvalue<=1.96))
					{ printf("\t\t%f \tN\t", tvalue); fflush(stdout); }
					else if(tvalue<-1.96)
					{ printf("\t\t%f \tA\t", tvalue); fflush(stdout); }
******/
/****** ******/ 
flgints='n'; /****** Converting the raw number of synonymous change to integer ******/
...
flgintn='n'; /****** Converting the raw number of nonsynonymous change to integer ******/
for(zza=0.0; zza<=10000.0; zza=zza+1.0)
{
...
******/
					...
					else if(totprob>=0.05)
					{
						...
						{
							printf("%f\tN", totprob); fflush(stdout);
							printf("\t%f\n", 1.0-totprob); fflush(stdout);
							++dnds;
/******
printf("%d\n", siten); fflush(stdout);
...
******/
					printf("nosubstitutions"); fflush(stdout);
					printf("\n"); fflush(stdout);
				}
			}
